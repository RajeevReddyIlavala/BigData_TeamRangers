...
for column in df.columns:	
	...
	for column in df.columns:
		...
		if int_type.count()!=0:
			...
			"stddev":int_type.select(F.stddev()).collect()[0][0]
			}
		...
		if float_type.count()!=0:
			...
			"stddev":float_type.select(F.stddev()).collect()[0][0]
			}
			column_json["data_types"].append(realJson)
		if date_type.count()!=0:
			dateTimeJson = {"type":"DATE/TIME",
			"count":date_type.count(),
			"max_value":, # yet to code -- waiting for date time data type identification
			"min_value":  # yet to code -- waiting for date time data type identification
			}
			column_json["data_types"].append(dateTimeJson)
		if string_type.count() !=0:
			s1 = string_type.select(column).sort(F.col(column).desc()).head(5)
			textJson = {"type":"TEXT",
			"count":string_type.count(),
			"shortest_values":, #yet to code
			"longest_values":,  #yet to code
			"average_length":   #yet to code
			}
